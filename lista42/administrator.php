...
<html>
	...
	<?php
		...
		if ( $_SESSION['admin_ok'] != 0 )
		{
			...
			else header('Location: http://localhost/lista42/zaloguj.php');
				...
				<tr<?php if ( $kursor % 2 == 0 ) echo ' style="background-color: rgba(0, 0, 0, 0.05)"'; ?>>
						...
						<td>
							<?php
								...
								$rezultat_zdjec = mysql_query( $zapytanie );
								
								while ( $zdjecie = mysql_fetch_assoc ( $rezultat_zdjec ) )
								{
									?><img style="height: 50px; width: 50px;" src="<?php echo $zdjecie['url'];?>"/><?php
								}
